...
| `web_request->httpHost` member access | Yes | Yes | Real startup folder (`site_setup_tags.inc`'s `botRedirect`, reached once `excludeBots` itself works) | Unsupported — newly discovered live-verifying the fixes above; `excludeBots` now registers, is called, and runs its full `with...do` loop before hitting this as the next distinct gap. Not yet scoped or implemented |
