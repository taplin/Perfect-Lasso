...
| Instance startup folder (`LassoStartup`, auto-loaded before any request) | Primary | Primary | A real instance's `LassoStartup` folder outside its site webroot | M2 implemented via `loadLassoStartupDirectory` + `LASSO_STARTUP_PATH`; 5 of 10 real files load cleanly, 5 hit distinct, cataloged legacy-syntax gaps — see `Documentation/lasso-perfect-server.md` |
...
| Legacy `define_tag('name', -flags) ... /define_tag` (parenthesized-call style) | Primary | No | Real startup folder (`_begin_tags.inc`'s `send_email2`, all of `getGeoIPInfo.inc`) | Unsupported |
| Legacy `define_type: 'name', 'base'; define_tag: 'name'; ... /define_tag; /define_type;` (colon-call style) | Primary | No | Real startup folder (all of `js_timer.inc`) | Unsupported |
| Top-level expression-bodied `define name => 'literal'` (no braces) | Different | Primary | Real startup folder (`paypal_express.inc`'s constant tags, `site_setup_tags.inc`'s `define br => '<br />'`) | Unsupported — parses as 3 separate top-level expressions ending in `unsupportedExpression("=>")`; blocks every later `define` in the same file since one throwing statement aborts the whole file's render |
| `with x in y do { ... }` iteration | No | Primary | Real startup folder (`excludeBots` itself, once `site_setup_tags.inc` can load past the gap above) | Unsupported |
