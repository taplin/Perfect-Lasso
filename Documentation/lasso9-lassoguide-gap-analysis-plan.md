...
- **✅ DONE — #7 `regexp` type + `string->findRegExp`/`->replaceRegExp`** (Section 5, `language-primitives-batch2` branch) — `NSRegularExpression`-backed, verified against Lasso 8.5 Language Guide Ch. 26's own worked examples. Two documented judgment calls: `String_ReplaceRegExp` returns a string despite Table 11's own description text saying "array" (a copy-paste artifact from the FindRegExp row above it — every worked example is a string); `RegExp->Split`'s trailing segment is omitted when empty (confirmed against the Guide's own `\W+`-on-a-sentence example) but the symmetric leading-segment case is undocumented and left as a disclosed assumption. Table 10's stateful "Interactive Find/Replace" tags (`->Find`/`->MatchString`/`->AppendReplacement`/etc., needing genuine mutable per-call position state unlike the stateless convenience surface implemented) are deliberately deferred. A real bug was found and fixed: the replacement-template translator only implemented Lasso's `\0`-`\9` group-placeholder syntax, not the equally-documented `$0`-`$9` alternate form, and mishandled the documented `\$` literal-dollar escape when a digit happened to follow it.
...
- **✅ DONE — #10 Type introspection (`->type()`, `->isA()`, `->isNotA()`, `->hasMethod()`)** (Section 6, `language-primitives-batch2` branch) — verified against Lasso 8.5 Ch. 43 Table 6 for `->Type`/`->IsA`, and lassoguide.com's Lasso 9 "Type/Object Introspection Methods" for `->IsNotA`/`->HasMethod` (absent from the 8.5 table). `->IsA`/`->IsNotA` implement Lasso 8.5's simpler boolean, flat exact-type-name-match semantics, NOT Lasso 9's richer integer 0-3 trait/parent-aware return value — this codebase has no type-inheritance/trait model anywhere (a custom `define_type`'s parent/base names are parsed but never acted on), so the richer form has nothing to check against. `->HasMethod` consults the real native/user-type registries for `.object` instances, and a hand-maintained mirror of `member()`'s own switch-case labels for primitives (disclosed as needing manual upkeep as `member()` grows). `->listMethods()`/`->parent()` from the table above are still open. A real regression was caught and fixed before merge: an early version gave these tags unconditional top priority and broke `.map`'s established key-first design (a file upload's literal `"type"` key must keep winning over the synthetic tag).
- **✅ DONE (partial) — `Var_Reset`/`Local_Reset`/`Global`/`Global_Reset`/`Global_Defined`/`Global_Remove`/`Globals`** (Ch. 15 "Variables", not previously in this doc's Top-10 — surfaced during the same batch) — `Global` didn't exist at all before this (only page-scoped `Variable`/`Var`/`Local` did), so `Global_Reset` needed the whole tag family built alongside it, backed by a genuinely separate storage namespace (`LassoContext.trueGlobals`) so a page `Variable` and a true `Global` sharing a name stay independent, matching the Guide's own worked example. **Known limitation, deliberately scoped**: `trueGlobals` lives for one `LassoContext` (one page render), not across requests server-wide — real Lasso's Globals are chiefly valuable *because* they persist across every page execution on the server (LassoStartup seeding, an entire "Asynchronous Access" section on concurrent cross-request safety), which this interpreter has no architecture for anywhere (no persistent server-wide process state exists in this codebase at all). What's implemented is closer to "a second per-page variable namespace" than the cross-request feature Ch. 15 is chiefly about — real cross-request Global persistence is unimplemented. **Not implemented at all**: the `@`/`[Reference]` variable-aliasing system (Ch. 15 pp. 230-232, Table 6) — two variable names sharing one mutable storage cell, `Var_Reset`'s actual "detaching references" behavior. Zero `@`-operator parser support exists anywhere (`ExpressionParser.swift`/`LassoParser.swift`), and `LassoContext`'s variable storage is plain, unboxed `LassoValue`s with no indirection layer — real aliasing needs both a new parser construct and a new storage-indirection layer, a materially different and larger kind of change than every other primitive gap closed this batch (all pure member-dispatch/free-tag additions). `Var_Reset`/`Local_Reset`/`Global_Reset` are therefore implemented as plain synonyms for their base tag — mathematically identical to "detach references, then set" in a codebase with no references to detach, not an approximation of it. A real regression was caught and fixed before merge: an earlier version of the `$name`-falls-back-to-a-global fix also (incorrectly) applied to `.unscoped`, which `var_defined`'s existing free function reads through — silently making `Var_Defined('x')` report `true` whenever an unrelated true Global named `x` existed, contradicting Ch. 15's own "current Lasso page" scoping for that tag.

Everything else below (String iteration/capture-dependent methods, the full Array/List/Set/Stack/Queue/Duration/Traits/Query-Expression subsystems, `@`/`[Reference]` aliasing, most of Error Handling beyond what's tracked) is still open, unchanged from the original analysis below.
